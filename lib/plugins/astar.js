...
(function () {
	...
	AStarInstance.prototype.getCollisionGroup = function() {
		...
		for (var groupIndex = 0; groupIndex < levelGroups.length; groupIndex++) {
			...
			if (this.debugLevel > 0) console.log("AStarInstance: TmxTileMap.levelGroup(",groupIndex,"): ",
						levelGroup.name);
			...
			for (var i = 0; i < levelGroup.children.length; i++) {
					...
					if (this.debugLevel > 0) console.log("AStarInstance: TmxTileMap.levelGroup(",groupIndex,").levelObjs(",i,"): ",
						levelChild.name,
						" id:",levelChild.id,
	...
	AStarInstance.prototype.showLevelObjects = function() {
		...
		for (var i = 0; i < levelObjs.length; i++) {
				...
				if (this.debugLevel > 0) console.log("AStarInstance: TmxTileMap.levelObjs(",i,"): ",
					levelChild.name,
					" id:",levelChild.id,
					" collissionType: ", collisionTypeStr);
				// debugger;
	...
	AStarInstance.prototype.copyTilesToLayerData = function() {
		...
		for (var i = 0; i < tmxLayers.length; i++) {
				...
				if (this.debugLevel > 0) console.log("AStarInstance: TmxTileMap.tmxLayer(",i,"): ", tmxLayer.name, " rows:",tmxLayer.rows," cols:",tmxLayer.cols);
				...
				if (tmxLayer.tilesets) {
					...
					for (var tilesetIndex = 0; tilesetIndex < tmxLayer.tilesets.length; tilesetIndex++) {
							...
							if (this.debugLevel > 0) console.log("AStarInstance: TmxTileMap.tmxLayer(",i,"): ", tmxLayer.name," tileset[",tilesetIndex,"]: ", currentTileset.name);
					}
				}
				...
				if ((typeof(tmxLayer.rows) !== "undefined") && (typeof(tmxLayer.cols) !== "undefined")) {
					...
					for (var y = 0; y < tmxLayer.rows; y++) {
							...
							if (this.debugLevel > 1) console.log("tmxLayer.layerData - row[",y,"]: ",rowStr);
					}
				}
		}
	...
	AStarInstance.prototype.init = function() {
		if (this.debugLevel > 0) console.log("AStarInstance: init");
		...
		if (this.debugLevel > 0) console.log("AStarInstance: TMXRenderer: rows:",tmxRenderer.rows," cols:",tmxRenderer.cols);
		...
		if (this.debugLevel > 0) console.log("AStarInstance: tilewidth:",this.tw," tileheight:",this.th);
				...
				for (var i = 0; (i < collisionGroup.children.length) && !foundCollision; i++) {
						...
						if (levelChild.body.shapes.length == 1) {
							...
							if (testRect.overlaps(bodyBounds)) {
									...
									if (this.debugLevel > 1) console.log("AStarInstance: collision at[",x,",",y,"]: id: ", levelChild.id);
									tile = layerData[x][y];
	...
	AStarInstance.prototype.search = function(x0,y0,x1,y1) {
		...
		//
		if (this.debugLevel > 0) console.log("astar.search: [",x0,",",y0,"] to [",x1,",",y1,"]")
		if (this.debugLevel > 0) console.log("astar.search: col,row[",~~(x0/this.tw),",",~~(y0/this.th),"] to [",~~(x1/this.tw),",",~~(y1/this.th),"]")
	...
	me.astar = me.plugin.Base.extend({
		...
		init: function() {
			if (this.debugLevel > 0) console.log("aStarPlugin: init");
		...
		refresh: function() {
			if (this.debugLevel > 0) console.log("aStarPlugin: refresh");
