...
(function () {
	...
	AStarInstance.prototype.init = function() {
		...
		var levelObjs = currentLevel.getObjects(true);
		for (var i = 0; i < levelObjs.length; i++) {
				var levelChild = levelObjs[i];
				// objects without names are probably colliders!
				console.log("AStarInstance: TmxTileMap.levelObjs(",i,"): ", levelChild.name, " id:",levelChild.id);
				// debugger;
		}
		...
		//var layerData = Array.apply(null, Array(this.rows * this.cols)).map(function () {});
		var layerData = Array.apply(null, Array(this.cols)).map(function () {return Array(this.rows);});
		var tilesets = [];
		//var tilesets = new TMXTilesetGroup();
